...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (!vcTracker.isValidDate(startDate) || !vcTracker.isValidDate(endDate)) {
      return reply(
        interaction,
        { content: '❌ Incorrect date format! Please use **YYYY-MM-DD** (e.g., `2026-09-01`).' },
        true
      );
    }
    if (startDate > endDate) {
      return reply(interaction, { content: '❌ The start date must be before the end date.' }, true);
    }
    ...
    if (Object.keys(stats.dailyMap).length) {
      breakdown = Object.keys(stats.dailyMap)
        .sort()
        .map((date) => `**${date}**: \`${Math.round((stats.dailyMap[date] / 3600) * 10) / 10} hrs\``)
        .join('\n');
    } else {
    ...
    const embed = categoryEmbed('utility', {
      ...
      fields: [
        { name: 'Total Calculated Time', value: `\`${totalHours.toFixed(2)} hrs\``, inline: true },
        ...
        { name: 'Period Status', value: statusStr, inline: false },
